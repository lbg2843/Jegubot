...
def _build_open_result(
    ...
    if last_snapshot is not None and last_snapshot.get("_ts") is not None:
        exit_price = _apply_exit_slippage(float(last_snapshot.get("_price") or 0.0), 0.0)
...
def simulate_trade(
    ...
    for snapshot in future_snapshots:
        ...
        if current_pnl_pct <= params.stop_loss_pct:
            exit_snapshot = snapshot
            exit_price = entry_price * (1.0 + params.stop_loss_pct / 100.0)
            exit_reason = "stop_loss"
            break

        if peak_pnl_pct >= params.trailing_activation_pct:
            trailing_active = True
        if trailing_active and drawdown_pct <= -params.trailing_stop_pct:
            exit_snapshot = snapshot
            exit_price = peak_price * (1.0 - params.trailing_stop_pct / 100.0)
            exit_reason = "trailing_stop"
            break

        if current_pnl_pct >= params.take_profit_pct:
            exit_snapshot = snapshot
    ...
    pnl_pct = _pct_change(exit_price or 0.0, entry_price)
    pnl_usd = position_size_usd * (pnl_pct / 100.0) - params.gas_fee_usd
